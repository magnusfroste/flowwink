...
- [Webinar Lifecycle & Lead Loop](mem://erp/webinar-lifecycle-and-lead-loop) — 5 lifecycle-RPCs + anonym `register_for_webinar` med auto lead-link/score, `webinar_tick` cron 5-min, alla skills MCP-exposade, admin-UI har lifecycle-knappar + attendance-toggle, events `webinar.{published,live,completed,cancelled,registered,attended}`.
