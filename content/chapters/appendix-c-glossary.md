...
| Term | Definition |
|------|-----------|
| **NemoClaw** | NVIDIA's fork of OpenClaw. Adds hardened sandboxing via OpenShell, policy-based access control, and NVIDIA hardware integration. |
| **NanoClaw** | Minimal, security-focused fork of OpenClaw. OS-level isolation, smaller footprint. |
| **DefenseClaw** | Enterprise governance layer for OpenClaw. Adds scanning, blocking, and audit logging for skills, MCP servers, and agent actions. |
| **Kilo Code** | Open-source agentic coding platform (VS Code/JetBrains extension + CLI). Model-agnostic, 500+ models. Used to write this handbook. Compatible with OpenClaw's AGENTS.md and SKILL.md formats. |
