...
We verified every claim about OpenClaw against the source code. We separated what is the reference model from what is Flowwink's adaptation. We documented NemoClaw, NanoClaw, DefenseClaw, and the full fork ecosystem with GitHub API data. We described how Claude Code, Cursor, Cline, and Roo work under the hood — and why thin wrapper is not a moat. We showed how governance, accountability, and the new Agent Manager role connect to McKinsey's framework and HBR's field reports.
