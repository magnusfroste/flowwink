...
/**
 * Contact Center — multi-skill router (Contact Center, Fas 1), internal handler.
 *
 * Moved from the standalone `contact-center` edge function (edge-surface
 * refactor B1a, wave 0). The edge function dispatched on `_skill` injected by
 * agent-execute's edge: dispatch; as an internal handler the skill name is
 * passed as a parameter instead — same dispatch, one less HTTP hop. All
 * response objects (including the {error} shapes) are unchanged.
 *
 * Skills:
...
export async function executeContactCenter(
  supabase: any,
  ...
  try {
    switch (skillName) {
      case "manage_channel":       return await manageChannel(supabase, args);
      case "send_channel_message": return await sendChannelMessage(supabase, args);
      case "handle_voicemail":     return await handleVoicemail(supabase, args);
      default:
        return { error: `Unknown contact-center skill: ${skillName || "(none)"}` };
    }
  } catch (err: any) {
