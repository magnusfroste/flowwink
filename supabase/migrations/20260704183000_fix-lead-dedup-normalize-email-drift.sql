-- Re-emit the CRM lead de-duplication functions (drift fix, not a logic change).
-- Live verification 2026-07-04 found normalize_email('x+tag@d') returning the
-- address UNCHANGED on rzhj — the plus-addressing strip was missing, so
-- find_duplicate_leads never collapsed gmail-style aliases (the headline feature
-- of this capability). Root cause is the same ledger drift as elsewhere this
-- session: the correct bodies live in migration 20260616120000, which is
-- back-dated below the managed instance's migration-ledger HEAD and was silently
-- skipped, leaving an older normalize_email in place (merge_leads happened to be
-- current — a drift MIX). Forward-dated CREATE OR REPLACE re-emits all three
-- canonical bodies verbatim so the whole feature is guaranteed current on every
-- instance. Idempotent.
