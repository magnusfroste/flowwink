-- Re-emit resolve_pricelist_price (drift fix, not a logic change). The correct,
-- ambiguity-free definition already exists in migration 20260613020000, but that
-- file is back-dated below the managed/forked instances' migration-ledger HEAD,
-- so it was silently skipped there — rzhj still runs an older body that fails at
-- runtime with 'column reference "price_cents" is ambiguous' (the RETURNS TABLE
-- output column collides with an unqualified products.price_cents in the query).
-- Caught by live verification 2026-07-04. Fix: forward-date the CURRENT correct
-- body (qualified pr.price_cents, output aliased to resolved_price, deepest
-- qualifying quantity break wins) so it actually reaches every instance.
-- Idempotent CREATE OR REPLACE; body identical to 20260613020000.
