-- ============================================================================
-- manage_account_roles(propose) died through the gateway: "DELETE requires a
-- WHERE clause".
--
-- The function clears its temp table with `DELETE FROM _their`. Supabase runs
-- PostgREST's role with the safeupdate extension, which rejects an unqualified
-- DELETE outright. Over psql — where every test of this function had been run —
-- safeupdate is off and the statement is fine. So the skill worked in
-- development and failed the first time an agent called it the way agents
-- actually call it: through the MCP gateway.
--
-- Worth naming as a class: a statement that is legal in one session and refused
-- in another is invisible to any test that only ever uses the permissive one.
-- The propose path is now TRUNCATE, which carries no such rule.
-- ============================================================================
...
BEGIN
  ...
  -- TRUNCATE, not DELETE. Supabase runs PostgREST's role with the safeupdate
  -- extension, which rejects an unqualified DELETE — so `DELETE FROM _their`
  -- raised "DELETE requires a WHERE clause" and the whole skill failed through
  -- the gateway while working perfectly over psql. Found on liteit 2026-08-10,
  -- the first time propose was called the way an agent actually calls it.
  TRUNCATE _their;
