...
DECLARE
  ...
  v_idx int := 1;
BEGIN
  -- Use existing leads as customers — don't fabricate if none exist
  FOR v_lead IN
  ...
  LOOP
    ...
    v_qnum := 'DEMO-Q-'||to_char(now(),'YYYY')||'-'||lpad((v_count+1)::text,4,'0');
    ...
    INSERT INTO quotes (
      ...
      v_lead.name, v_lead.email, v_lead.company,
    ...
    -- Add 2-3 line items using real products if available
    v_idx := 0;
    FOR v_product IN
    ...
    -- Fallback line if no products exist
    IF v_idx = 0 THEN
