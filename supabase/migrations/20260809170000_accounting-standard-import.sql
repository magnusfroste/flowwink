...
--
-- Today's find made the argument. Our own "BAS 2024" chart was hand-written —
-- 166 wrong names, 40 invented accounts — because loading a standard was a
-- developer task nobody actually did. The fix (source artifact + checksum +
-- gate) works, but it still assumes WE do it per country. That does not scale
-- to SKR03, and it cannot anticipate what a specific company books.
--
-- So the platform becomes the container + the gate, and the agent does the
-- loading:
--
--   import_accounting_standard   The agent has the official file (it can read
--                                xlsx; we should not parse uploads server-side).
--                                It sends structured rows + provenance. We
--                                validate, store, and wire the role layer so
--                                the engine can post. FAIL CLOSED: no source
--                                URL + sha256, no import — an unsourced
--                                standard is how today's mess was born.
--
--   propose_posting_templates    Posting templates are NOT published anywhere —
--                                they describe what a company actually does,
--                                which FlowWink cannot foresee. An agent that
--                                holds the company's transaction history
--                                authors them; the STRUCTURAL gate verifies:
--                                lines balance, accounts exist in the chart,
--                                names are the chart's (corrected, reported).
--                                Rejected templates come back with reasons —
--                                partial success by design.
--
-- The engine never branches on country (Law: country is data). These two
-- functions are how the data arrives without a developer in the loop.
-- ============================================================================

-- ── The schema itself contradicted the container vision ─────────────────────
-- chart_of_accounts had UNIQUE (account_code) GLOBALLY, so no two locales could
-- share a code. SKR03's 1200 is "Bank"; BAS 2024's 1200 is "Inventarier" — both
-- are correct, and the first live import proved it: 9 of 55 German accounts
-- vanished into ON CONFLICT DO NOTHING while the response said imported:true.
-- Uniqueness is per locale. Only historical migrations targeted the old
-- constraint (verified by grep); no runtime code does.
...
CREATE OR REPLACE FUNCTION public.import_accounting_standard(
  -- Every parameter after p_locale defaults to NULL on purpose: an agent that
  -- omits one must reach the validation errors that TEACH (why provenance is
  -- required), not PostgREST's "no matching function" signature error.
...
BEGIN
  ...
  -- ── Provenance first. This is the whole lesson of 2026-08-09: a chart that
  -- cannot say where it came from is a chart nobody can ever verify again. ──
  ...
  -- ── Roles: without them the locale is inert — the engine posts through
  -- account_for(role), never through hardcoded numbers. ──
  ...
  -- ── Refuse an accidental overwrite of a locale that already has a chart ──
  ...
  FOR v_acc IN SELECT * FROM jsonb_array_elements(p_accounts) LOOP
    ...
    ELSE
      -- No ON CONFLICT here on purpose. The first live run swallowed 9 accounts
      -- through DO NOTHING while answering imported:true — a genuine conflict
      -- must RAISE, because a write that silently did not happen is the one
      -- failure an importer may never absorb.
  ...
  -- Arithmetic honesty: every delivered account is inserted, updated or
  -- verifiably unchanged. Anything else is a bug that must surface, not a
  -- number that quietly disagrees with the payload.
  ...
  -- ── Provenance, so the next reader can verify instead of trusting ──
  ...
  FOR v_tpl IN SELECT * FROM jsonb_array_elements(p_templates) LOOP
    ...
    ELSE
      FOR v_line IN SELECT * FROM jsonb_array_elements(v_tpl -> 'template_lines') LOOP
        ...
        -- The chart is the truth for names. A template that carries its own
        -- wording is a second copy that drifts — the exact mechanism that let
        -- four VAT templates keep a wrong account for months.
    ...
    -- is_system=false: agent-proposed templates belong to the operator, who may
    -- edit or delete them. System templates ship with the pack. The insert is
    -- fenced per template: one template's database error becomes ITS rejection
    -- reason, never the whole batch's death — the first live run lost three
    -- valid templates to one NOT NULL violation.
    BEGIN
