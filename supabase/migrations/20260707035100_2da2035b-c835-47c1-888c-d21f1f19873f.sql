-- Feature: Calendar event reminders. Adds an 8th job to register_flowpilot_cron
-- (identical pattern to the booking-reminders job in 20260703152000) that hits
-- send-calendar-reminders every 15 minutes.
...
BEGIN
  ...
  -- 3. Publish scheduled pages (every 5 minutes)
  ...
  -- 7. Booking reminders (every 15 minutes)
  ...
  -- 8. Calendar event reminders (every 15 minutes) — sweeps calendar_events with
  --    reminder_minutes set that fall within the reminder window and emails
  --    every attendee. reminder_sent_at is stamped in the edge function.
  SELECT EXISTS(SELECT 1 FROM cron.job WHERE jobname = 'calendar-reminders') INTO job_exists;
  IF NOT job_exists THEN
    PERFORM cron.schedule(
      'calendar-reminders',
      '*/15 * * * *',
      format(
        'SELECT net.http_post(url := %L, headers := %L::jsonb, body := ''{"source": "cron"}''::jsonb) AS request_id;',
        p_supabase_url || '/functions/v1/send-calendar-reminders',
        auth_header
      )
    );
    result := result || '{"calendar_reminders": "registered"}'::jsonb;
  ELSE
    result := result || '{"calendar_reminders": "already_exists"}'::jsonb;
  END IF;
