-- publish-scheduled-pages cron fix (fleet incident 2026-07-17).
--
-- The registrar scheduled an HTTP call to /functions/v1/publish-scheduled-pages —
-- an edge function that does not exist (the logic lives in the DB function
-- public.publish_scheduled_pages()). Result: a silent 404 every 5 minutes on
-- liteit/www/autoversio since setup, and scheduled pages never auto-published.
-- Fix: the cron job runs the DB function directly — no HTTP, no auth, no edge
-- dependency. Idempotent + forward-dated (managed ledgers skip backdated files).

-- 1) Repair the job in place wherever it already exists with the broken command.
DO $fix$
BEGIN
  IF EXISTS (
    SELECT 1 FROM cron.job
    WHERE jobname = 'publish-scheduled-pages' AND command LIKE '%functions/v1/publish-scheduled-pages%'
  ) THEN
    PERFORM cron.alter_job(
      (SELECT jobid FROM cron.job WHERE jobname = 'publish-scheduled-pages'),
      command := 'SELECT public.publish_scheduled_pages();'
    );
  END IF;
END
$fix$;

-- 2) Fix the registrar so NEW instances schedule the SQL call from the start.
--    Byte-identical to the previous definition except branch 3.
